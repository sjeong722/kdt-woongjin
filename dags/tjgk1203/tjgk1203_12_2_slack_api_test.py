...
with DAG(
    dag_id='tjgk1203_12_2_slack_api_test_dag',
    start_date=pendulum.today('UTC').add(days=-1),
    schedule='0 0 * * *',
    catchup=False,
    tags=['tjgk1203', 'slack', 'api'],
) as dag:
    ...
    send_slack = SlackAPIPostOperator(
        ...
        slack_conn_id='tjgk1203_slack_conn',
        channel='#bot-playground',  # 보낼 채널명을 입력하세요 (예: #general)
        text=':rocket: Airflow -> Slack API (Token) 연결 성공! 서하 봇 테스트 DAG입니다.',
        username='Seoha bot', 
    )
